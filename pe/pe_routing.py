...
def get_routing_handler():
    ...
    return process_packetfrom scapy.all import sniff, send, IP, TCP, UDP, conf
from misc.grab_config import get_config
from misc.get_route import dig_RIB
from misc.logging import handle_log
import ipaddress
...
def start_routing():
    ...
    config = get_config(["pe_route_dest", "iface"])

    pe_route_dest = config["pe_route_dest"]
    iface = config.get("iface", None)
